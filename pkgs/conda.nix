...
buildFHSUserEnv {
  ...
  profile = ''
    ...
    __conda_setup="$('/home/despera/.conda/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
    if [ $? -eq 0 ]; then
        eval "$__conda_setup"
    else
        if [ -f "/home/despera/.conda/etc/profile.d/conda.sh" ]; then
            . "/home/despera/.conda/etc/profile.d/conda.sh"
        else
            export PATH="/home/despera/.conda/bin:$PATH"
        fi
    fi
    unset __conda_setup
    # <<< conda initialize <<<
